<?php
require_once( dirname(__FILE__).'/import-queries_rsu.php' );
...
class RS_PP_Analysis {
	...
	function analyze() {
		...
		$compat_captions = array( 
			'core' => 		 __( 'Manually configurable with PP Core; Auto-importable with PP Pro', 'rsu' ),
			'core-manual' => __( 'Manually configurable with PP Core', 'rsu' ),
			'core-api' => 	 __( 'Require use of PP Core API', 'rsu' ),
			'pro' => 		 __( 'Auto-importable with PP Pro', 'rsu' ),
			'pro-manual' =>  __( 'Manually configurable with PP Pro', 'rsu' ),
			'pro-api' => 	 __( 'Require use of PP Pro API', 'rsu' ),
			'consult' => 	 __( 'Not supported by Press Permit; Paid consulting may be available', 'rsu' ),
		);
		
		?>
		<!-- <h1><?php _e( 'RS Migration Advisor', 'rsu' );?></h1> -->
		<h2 style="margin-bottom: 25px"><?php printf( __( 'Analyzing your Role Scoper installation&apos;s %1$sPress Permit%2$s compatibility...', 'rsu' ), '<a href="#pp-list-end">', '</a>' );?></h2>
		<?php
		...
		foreach ( $blog_ids as $id ) {
			...
			foreach( array_keys( $this->compat ) as $ctype ) :?>
				...
				?>
					<div class="rsu-issue"><a href="#"><?php printf( __( '(%1$d) %2$s:', 'rsu' ), count($this->compat[$ctype]), $compat_captions[$ctype] );?></a>
					<ul<?php echo $style;?>>
		...
		jQuery(document).ready( function($) {
			$('.rsu-issue a').click( function() {
				$(this).parent().find('ul').show();
				return false;
			});
			$('.rsu-issue ul').click( function() {
				$(this).hide();
				return false;
			});
		});
		...
		<br />
		<h3><?php _e( 'Further information on <a href="#pp-list-end">Press Permit</a> :', 'rsu' );?></h3>
		
		<?php
		$img_url = plugins_url( '', RSU_FILE ) . '/admin/images/';
		$lang_id = 'rsu';
		?>
		<script type="text/javascript">
		/* <![CDATA[ */
		...
		<style>
		#pp_features {text-align:left;border:1px solid #eee;margin:5px 20px 5px 2px}
		...
		<div id="pp_features"><div class="pp-logo"><a href="http://presspermit.com"><img src="<?php echo $img_url;?>pp-logo.png" /></a></div><ul class="pp-features">
		...
		<ul class="rsu-pplinks">
		<li>
		<span class="plugins update-message"><a href="<?php echo rsu_plugin_info_url('press-permit-core');?>" class="thickbox" title="<?php printf( __('%s (free install)', $lang_id), 'Press Permit Core' );?>"><?php _e('Install PP Core', 'rsu');?></a></span>
		</li>
		...
		foreach( $rs_groups as $rs_group ) {
			...
			if ( empty($metagroup_type) || ( $metagroup_type != 'wp_role' ) ) {	// RS stores WP role group membership alongside RS role assignments, but PP activation has already inserted equivalent records in pp_group_members
				...
				if ( ! empty( $group_members['consult'] ) )
					$this->compat['consult'][]= sprintf( _n( 'Group "%1$s": %2$d membership requests/recommendation', 'Group "%1$s": %2$d membership requests/recommendations', $group_members['consult'], 'rsu' ), $rs_group->group_name, $group_members['consult'] );
			}
		}
		
		if ( $rs_groups ) {
			$this->compat['core'][]= sprintf( _n( '%s Role Group', '%s Role Groups', count( $rs_groups ), 'rsu' ), count( $rs_groups ) );
			
			if ( ! empty( $group_members['core'] ) )
				$this->compat['core'][]= sprintf( _n( '%s Group Member', '%s Group Members', $group_members['core'], 'rsu' ), $group_members['core'] );
		}
	}
	...
	function rs_site_roles() {
		...
		//dump($this->wp_role_caps);
		//dump($this->rs_role_caps);
		...
		foreach( array( 'forbidden_taxonomies_rs', 'default_disable_taxonomies_rs', 'default_options_rs', 'default_otype_options_rs', 'options_rs' ) as $hook ) {
			if ( has_filter( $hook ) )
				$this->compat['core-api'][]= sprintf( __( 'Third party function hooked to filter %s', 'rsu' ), $hook );
		}
		
		foreach( array( 'options_sitewide_rs', 'site_options_rs' ) as $hook ) {
			if ( has_filter( $hook ) )
				$this->compat['pro-api'][]= sprintf( __( 'Third party function hooked to filter %s', 'rsu' ), $hook );
		}
		...
		if ( $ok_role_count )
			$this->compat['core'][]= sprintf( _n( '%s General Role', '%s General Roles', $ok_role_count, 'rsu' ), $ok_role_count );

		if ( $cdate_count )
			$this->compat['consult'][]= sprintf( _n( '%s General Role with content date limits', '%s General Roles with content date limits', $cdate_count, 'rsu' ), $cdate_count );

		if ( $date_lim_count )
			$this->compat['pro-manual'][]= sprintf( _n( '%s General Role with role date limits', '%s General Roles with role date limits', $date_lim_count, 'rsu' ), $date_lim_count );
			
		if ( $manual_roles ) {
			foreach( $manual_roles as $role_name => $role_count )
				$this->compat['pro-manual'][]= sprintf( _n( '%1$s General Role of "%2$s" (n/a role)', '%1$s General Roles of "%2$s" (n/a role)', $role_count, 'rsu' ), $role_count, $role_name );
		}

		if ( $customized_roles ) {
			foreach( $customized_roles as $role_name => $role_count )
				$this->compat['core-manual'][]= sprintf( _n( '%1$s General Role of "%2$s" (customized RS or WP role caps)', '%1$s General Roles of "%2$s" (customized RS or WP role caps)', $role_count, 'rsu' ), $role_count, ucwords( str_replace( '_', ' ', $role_name ) ) );
		}
		
		if ( $unsupported_roles ) {
			foreach( $unsupported_roles as $role_name => $role_count )
				$this->compat['consult'][]= sprintf( _n( '%1$s General Role of "%2$s" (unsupported role)', '%1$s General Roles of "%2$s" (unsupported role)', $role_count, 'rsu' ), $role_count, $role_name );
		}
	}
	...
	function rs_restrictions() {
		...
		*/
		...
		if ( $core_count )
			$this->compat['core'][]= sprintf( _n( '%s Reader Restriction', '%s Reader Restrictions', $core_count, 'rsu' ), $core_count );
		
		if ( $pro_count )
			$this->compat['pro'][]= sprintf( _n( '%s Editing Restriction', '%s Editing Restrictions', $pro_count, 'rsu' ), $pro_count );
		
		if ( $unsupported_roles ) {
			foreach( $unsupported_roles as $role_name => $role_count )
				$this->compat['consult'][]= sprintf( _n( '%1$s Restriction of "%2$s" (unsupported role definition)', '%1$s Restrictions of "%2$s" (unsupported role definition)', $role_count, 'rsu' ), $role_count, $role_name );
		}
		...
		if ( $core_count )
			$this->compat['core'][]= sprintf( _n( '%s Default Reader Restriction', '%s Default Reader Restrictions', $core_count, 'rsu' ), $core_count );
		
		if ( $pro_count )
			$this->compat['pro'][]= sprintf( _n( '%s Default Editing Restriction', '%s Default Editing Restrictions', $pro_count, 'rsu' ), $pro_count );
		
		if ( $unsupported_roles ) {
			foreach( $unsupported_roles as $role_name => $role_count )
				$this->compat['consult'][]= sprintf( _n( '%1$s Default Restriction of "%2$s" (unsupported role)', '%1$s Default Restrictions of "%2$s" (unsupported role)', $role_count, 'rsu' ), $role_count, $role_name );
		}
	...
	function rs_item_roles() {
		...
		if ( $core_count )
			$this->compat['core'][]= sprintf( _n( '%s Term / Object Reader Role', '%s Term / Object Reader Roles', $core_count, 'rsu' ), $core_count );
		
		if ( $pro_count )
			$this->compat['pro'][]= sprintf( _n( '%s Term / Object Editing Role', '%s Term / Object Editing Roles', $pro_count, 'rsu' ), $pro_count );
		
		if ( $group_mgr_count )
			$this->compat['pro'][]= sprintf( _n( '%s Group Administrator Assignment', '%s Group Administrator Assignments', $group_mgr_count, 'rsu' ), $group_mgr_count );
		
		if ( $unsupported_roles ) {
			foreach( $unsupported_roles as $role_name => $role_count )
				$this->compat['consult'][]= sprintf( _n( '%1$s Term / Object assignment of "%2$s" (unsupported role)', '%1$s Term / Object assignments of "%2$s" (unsupported role)', $role_count, 'rsu' ), $role_count, $role_name );
		}
	}
	...
	function rs_options() {
				...
				foreach( array_keys( $check_netwide_options ) as $opt_name ) {
					if ( isset($this->rs_default_otype_options[$opt_name]) ) {
						if ( isset($this->rs_net_options['scoper_' . $opt_name]) && $this->rs_default_otype_options[$opt_name] != $this->rs_net_options['scoper_' . $opt_name] ) {
							$this->compat['core-api'][]= sprintf( __( 'Customized network default for option array %s', 'rsu' ), $opt_name );
						}
						...
						if ( isset($this->rs_net_options['scoper_' . $opt_name]) && $this->rs_default_options[$opt_name] != $this->rs_net_options['scoper_' . $opt_name] ) {
							$this->compat['core-api'][]= sprintf( __( 'Customized network default for option %s', 'rsu' ), $opt_name );
						}
					}
				}
			}
		...
		if( $this->rs_get_otype_option( 'do_teaser', 'post') ) {
			$this->compat['pro'][]= __( 'Pro Feature: Hidden Content Teaser', 'rsu' );
				...
				foreach( $teaser_types as $type ) {
					if ( $first )
						...
						if ( $main_setting != $this->rs_get_otype_option( 'teaser_' . $opt, 'post', $type ) ) {
							$this->compat['pro-api'][]= sprintf( __( 'Type-specific option requires use of plugin API : Teaser %s', 'rsu' ), ucwords( str_replace( '_', ' ', $opt ) ) );
							break;
						}
					}
		...
		if( $arr = (array) $this->rs_get_option( 'default_private') ) {
			foreach ( $arr as $val ) {
				if ( $val ) {
					$this->compat['pro'][]= __( 'Pro Feature: Default Privacy', 'rsu' );
					break;
				}
			}
		}
		...
		if( $this->rs_get_option( 'file_filtering') && ( ! defined( 'DISABLE_ATTACHMENT_FILTERING' ) || ! DISABLE_ATTACHMENT_FILTERING ) )
			$this->compat['pro'][]= __( 'Pro Feature: File Filtering', 'rsu' );
		
		if( $this->rs_get_option( 'lock_top_pages') )
			$this->compat['pro'][]= __( 'Pro Feature: Lock Top Pages', 'rsu' );
		
		if( $this->rs_get_option( 'group_requests') )
			$this->compat['consult'][]= __( 'Unsupported Option: Role Group Requests', 'rsu' );
		
		if( $this->rs_get_option( 'group_recommendations') )
			$this->compat['consult'][]= __( 'Unsupported Option: Role Group Recommendations', 'rsu' );
		
		if( $this->rs_get_option( 'hide_non_editor_admin_divs') )
			$this->compat['pro'][]= __( 'Pro Feature: Limited Editing Elements', 'rsu' );
		
		if ( $this->rs_get_option( 'admin_nav_menu_filter_items' ) )
			$this->compat['pro'][]= __( 'Pro Feature: Nav Menu Management - list only user-editable content as available items', 'rsu' );
		
		if( $this->rs_get_option( 'feed_link_http_auth') )
			$this->compat['consult'][]= __( 'Unsupported Option: HTTP Authentication Request in RSS Feed Links', 'rsu' );
		
		if( $this->rs_get_option( 'role_content_date_limits') )
			$this->compat['consult'][]= __( 'Unsupported Option: Enable Content Date Limits for Roles', 'rsu' );
			
		if( $this->rs_get_option( 'role_duration_limits') )
			$this->compat['pro-manual'][]= __( 'Manual Configuration Required: Role Duration Limits', 'rsu' );
			
		if( $this->rs_get_option( 'group_recommendations') )
			$this->compat['pro-manual'][]= __( 'Option requires manual reconfig: Role Duration Limits', 'rsu' );
		
		$this->compat['pro'][]= __( 'Pro Feature: Media Library Filtering (based on parent post access)', 'rsu' );
		
		//if ( ! $this->rs_get_option( 'admin_others_attached_files') )
			$this->compat['pro'][]= __( 'Pro Option: Non-editors see other users&apos; attached uploads', 'rsu' );
		
		//if ( ! $this->rs_get_option( 'admin_others_unattached_files') )
			$this->compat['pro'][]= __( 'Pro Option: Non-editors see other users&apos; unattached uploads', 'rsu' );
		
		if ( defined( 'SCOPER_CONTENT_ADMIN_CAP' ) )
			$this->compat['core-manual'][]= __( 'Manual configuration: customized content administration capability', 'rsu' );
			
		if ( defined( 'SCOPER_USER_ADMIN_CAP' ) )
			$this->compat['core-manual'][]= __( 'Manual configuration: customized user administration capability', 'rsu' );
			
		if ( defined( 'SCOPER_OPTION_ADMIN_CAP' ) )
			$this->compat['core-manual'][]= __( 'Manual configuration: customized option administration capability', 'rsu' );
		
		if ( ! $this->rs_get_option( 'limit_user_edit_by_level') )
			$this->compat['pro'][]= __( 'Pro Option: Limit user edit by level', 'rsu' );
		
		if ( ! $this->rs_get_option( 'define_usergroups') )
			$this->compat['core-manual'][]= __( 'Manual Configuration: disable User Groups', 'rsu' );
		
		if ( ! $this->rs_get_option( 'enable_group_roles') )
			$this->compat['consult'][]= __( 'Unsupported Option: disable Group Roles', 'rsu' );
			
		if ( ! $this->rs_get_option( 'enable_user_roles') )
			$this->compat['consult'][]= __( 'Unsupported Option: disable User Roles', 'rsu' );
			
		if ( $arr = $this->rs_get_option( 'disabled_access_types') ) {
			foreach( $arr as $val ) {
				if ( $val ) {
					$this->compat['consult'][]= __( 'Unsupported Option: disabled Access Types', 'rsu' );
					break;
				}
			}
		}
		...
		if ( $this->rs_get_option( 'no_frontend_admin') )
			$this->compat['core-api'][]= __( 'Option requires use of plugin API: Assume No Front-end Admin', 'rsu' );
		
		if ( $this->rs_get_otype_option( 'limit_object_editors', 'post', 'post' ) || $this->rs_get_otype_option( 'limit_object_editors', 'post', 'page' ) )
			$this->compat['consult'][]= __( 'Unsupported Option: Limit eligible users for Post-specific editing roles', 'rsu' );
		
		if ( ! $this->rs_get_otype_option( 'private_items_listable', 'post', 'page' ) )
			$this->compat['consult'][]= __( 'Unsupported Option: Omit private pages from listing even if logged user can read them', 'rsu' );
		
		if ( $this->rs_get_option( 'require_moderate_comments_cap' ) )
			$this->compat['consult'][]= __( 'Unsupported Option: Require moderate_comments capability', 'rsu' );
		
		if ( MULTISITE ) {
			if ( ! $this->rs_get_option( 'mu_sitewide_groups') )
				$this->compat['pro'][]= __( 'Pro Option: Network-wide role groups', 'rsu' );
		}
		...
		*/
		...
		$this->compat['core'][]= __( 'Options: Post Type Usage, Taxonomy Usage, Private Caption, Display Hints, User Profile Group/Role Display, Create Posts Cap, Role Assignment Roles', 'rsu' );
	}
